    ...
    def _iter_items(cls, repo, common_path=None, with_destination=False):
        ...
        for dest, rela_path in cls._iter_packed_refs(repo):  # @UnusedVariable
            if rela_path.startswith(common_path):
                rela_paths.add(rela_path)
                if with_destination and not rela_paths_dest.has_key(rela_path):
                    rela_paths_dest[rela_path] = dest
        ...
        for path in sorted(rela_paths):
            try:
                if with_destination:
                    yield cls.from_path(repo, path), rela_paths_dest[path] if rela_paths_dest.has_key(path) else None
                else:
